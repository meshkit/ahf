function isborder = determine_border_vertices_mex(nv, elems)
% DETERMINE_BORDER_VERTICES_MEX Determine border vertices of a mesh.
%
% ISBORDER = DETERMINE_BORDER_VERTICES_MEX(NV,ELEMS)
% Determines border vertices of a mesh.  It supports both linear
%    and quadratic elements. It returns bitmap of border vertices. For
%    quadratic elements, vertices on edge and face centers are set
%    to false, unless QUADRATIC is set to true at input.
%
% Example
% ISBORDER = DETERMINE_BORDER_VERTICES_MEX(NV,ELEMS)
%
...
if size(elems,2)==1
    ...
    while offset < size(elems,1)
        switch elems(offset)
            ...
            case {5,14}
                ...
                for jj=1:5
                    if opphfs(offset_o+jj) == 0
                        nvpf = (3+int32(jj==1))*(1+isquadratic)+int32(isquadratic && jj==1);
                        for kk=1:nvpf
            ...
            case {6,15,18}
                ...
                for jj=1:5
                    if opphfs(offset_o+jj) == 0
                        nvpf = (3+(jj<4))*(1+isquadratic)+int32(elems(offset)==18 && jj<4);
                        for kk=1:nvpf
    ...
    switch size(elems,2)
        case {4,10}
            hf = int32([1,3,2,7,5,6; 1,2,4,5,9,8; 2,3,4,6,10,9; 3,1,4,7,8,10]);
            isquadratic = int32(quadratic && size(elems,2)>4);
            nvpf = 3 * (1+isquadratic);
        case {5,14}
            hf_pyr = int32([1,4,3,2,9,8,7,6,14; 1,2,5,5,6,11,10,10,10; ...
                2,3,5,5,7,12,11,11,11; 3,4,5,5,8,13,12,12,12; ...
                4,1,5,5,9,10,13,13,13]);
            hf = hf_pyr;
            isquadratic = int32(quadratic && size(elems,2)>5);
            nvpf = 4 * (1+isquadratic) + 1;
        case {6,15,18}
            hf = int32([1,2,5,4,7,11,13,10,16; 2,3,6,5 8,12,14,11,17; ...
                3,1,4,6,9,10,15,12,18; 1,3,2,2,9,8,7,7,7; ...
                4,5,6,6,13,14,15,15,15]);
            isquadratic = int32(quadratic && size(elems,2)>6);
            nvpf = 4 * (1+isquadratic) + int32(size(elems,2)==18);
        case {8,20,27}
            hf = int32([1,4,3,2,12,11,10,9,21; 1,2,6,5,9,14,17,13,22; ...
                2,3,7,6,10,15,18,14,23; 3,4,8,7,11,16,19,15,24; ...
                4,1,5,8,13,20,16,12,25; 5,6,7,8,17,18,19,20,26]);
            isquadratic = int32(quadratic && size(elems,2)>8);
            nvpf = 4 * (1+isquadratic) + int32(size(elems,2)==27);
        otherwise
            hf = int32(0); nvpf = int32(0); %#ok<NASGU>
