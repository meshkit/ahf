...
% [B2V,BDEDGS,EDGMAP] = EXTRACT_BORDER_CURV(NV,ELEMS,FLABEL,OPPHES,NF)
...
% [B2V,BDEDGS,EDGMAP] = EXTRACT_BORDER_CURV(NV,ELEMS,FLABEL,OPPHES)
% [B2V,BDEDGS,EDGMAP] = EXTRACT_BORDER_CURV(NV,ELEMS,FLABEL,OPPHES,INWARDS)
...
if nargin<3
    if size(elems,2)==3
        [b2v, bdedgs, edgmap] = extract_border_curv_tri(nv, elems, flabel, opphes, inwards);
    else
        [b2v, bdedgs, edgmap] = extract_border_curv_quad(nv, elems, flabel, opphes, inwards);
    end
else
    if size(elems,2)==3
        [b2v, bdedgs, edgmap] = extract_border_curv_tri(nv, elems, flabel, opphes, inwards);
