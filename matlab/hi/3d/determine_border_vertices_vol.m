function isborder = determine_border_vertices_vol(nv, elems, opphfs, ...
    isborder, quadratic) %#codegen
...
if nargin<3; opphfs = determine_opposite_halfface(nv, elems); end
if nargin<4 || isempty(isborder)
    isborder = false(nv,1);
else
    assert(size(isborder,1)>=nv);
end
if nargin<5; quadratic=false; end
...
if size(elems,2)==1
    ...
    switch size(elems,2)
        ...
        otherwise
            hf = hf_pyr; nvpf = int32(0); %#ok<NASGU>
