...
if (zonetype == 2) % Structured
    ...
    if (iphysdim ==2)
        ...
        rmax = zeros(1,2,'int32');
        rmax(1,1:2) = sz(1,1:2);  % upper range index of vertices
    elseif (iphysdim == 3)
        rmin = [1 1 1];  % lower range index of vertices
        rmax = zeros(1,3,'int32');
        rmax(1,1:3) = sz(1,1:3);  % upper range index of vertices
    else
    ...
    if (iphysdim == 2)
        ...
        elem_end = int32(sz(4:5)); % upper range index of elements
        % elem_end = sz(1:2) + 1; % upper range of elements with ghost cells
    elseif (iphysdim == 3)
        elem_start = [1 1 1]; % lower range index of elements
        elem_end = sz(4:6); % upper range index of elements
        % elem_end = sz(1:3) + 1; % upper range of elements with ghost cells
    else
        error('physical dimension not supported');
    end
    
    % Store dimensions of structured grid
    elems = zeros(1,9,'int32');
    if (iphysdim == 2)
        elems(1:2) = sz(1:2);
    elseif (iphysdim == 3)
        elems(1:3) = sz(1:3);
...
for ii =1:length(fieldlist)
    ...
    if (iphysdim == 2)
        switch (in_struct(index_struct).datatype )
            case 2
                out_var_str.(var) = zeros(index_max(1),index_max(2),in_struct(index_struct).ncol,'int32');
            case 3
                out_var_str.(var) = zeros(index_max(1),index_max(2),in_struct(index_struct).ncol,'single');
            case 5
    ...
    elseif (iphysdim == 3)
        switch (in_struct(index_struct).datatype )
            case 2
                out_var_str.(var) = zeros(index_max(1),index_max(2),index_max(3),in_struct(index_struct).ncol,'int32');
            case 3
                out_var_str.(var) = zeros(index_max(1),index_max(2),index_max(3),in_struct(index_struct).ncol,'single');
            case 5
...
%! if(~isequal(xs,xs1)|| ~isequal(tris, tris1) || ~isa(tris1,'int32'))
