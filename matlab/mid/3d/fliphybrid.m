function [inset,eltset,xs_hyb,elems_buf,elems_type,elems_offsets,...
  reg_opphfs,cum_flips]=...
  fliphybrid(inset,eltset,xs_hyb,elems_buf,elems_type,elems_offsets,...
  reg_opphfs,node_constraints,dothree2two,dotwo2three,dofour2four,...
...
while nflipped_tot>0
  ...
  if(dotwo2three)
    ...
    [live_elements,nflips,elems_buf,elems_type elems_offsets,...
      ...
      reg_opphfs,xs_hyb,int32(live_elements),ninset,nelements);
  ...
  if(nflipped_tot>0) 
    ...
    reg_opphfs(live_elements+1:end,:)=[];
    eltset(ninset+1:end)=[];
  end
  ...
  if(dofour2four)
    tic
    [nflips elems_buf,elems_type,elems_offsets,reg_opphfs]=...
