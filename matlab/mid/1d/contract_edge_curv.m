function [nv, ne, edgs, sibhvs, v2hv, vvs, evs] = contract_edge_curv(eid, nv, ne, ...
...
%   CONTRACT_EDGE_CURV(EID, NV, NE, EDGS, SIBHVS, V2HV, VVS, EVS) contracts edge
...
prev_eid = hvid2eid(sibhvs(eid,1)); prev_lid = hvid2lvid(sibhvs(eid,1));
next_eid = hvid2eid(sibhvs(eid,2)); next_lid = hvid2lvid(sibhvs(eid,2));

% Open curve with a single edge
if ~sibhvs(eid,1) && ~sibhvs(eid,2) 
...
%TODO: Implement this branch
    assert(false);    
elseif edgs(hvid2eid(sibhvs(eid,1)), 3-hvid2lvid(sibhvs(eid,1))) == ...
        edgs(hvid2eid(sibhvs(eid,2)), 3-hvid2lvid(sibhvs(eid,2)))
...
%TODO: Debug this branch
    ...
    [edgs, sibhvs, v2hv] = swap_vertices_curv(org, nv, edgs, sibhvs, v2hv);
    if nargin>6
    ...
    [edgs, sibhvs, v2hv] = swap_edges_curv(eid, ne, edgs, sibhvs, v2hv);
    if nargin>7
    ...
    sibhvs(prev_eid, prev_lid) = 2*next_eid + next_lid-1;
    sibhvs(next_eid, next_lid) = 2*prev_eid + prev_lid-1;
