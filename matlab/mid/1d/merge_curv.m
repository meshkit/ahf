function [nv, ne, edgs, opphvs, v2hv, vvs, evs] = merge_curv(vid1, vid2, nv, ne, ...
    edgs, opphvs, v2hv, vvs, evs) %#codegen 
%MERGE_CURV    Merge curves locally.
%   [NV, NE, EDGS, OPPHVS, V2HV, VVS, EVS] = ....
%   MERGE_CURV(HVID1, HVID2, NV, NE, EDGS, OPPHVS, V2HV, VVS, EVS)
...
d1 = distance( eid1, eid2, opphvs, MAXDIST);
d2 = distance( eid2, eid1, opphvs, MAXDIST);
...
if d1 <= MAXDIST || d2 <= MAXDIST
    ...
    elseif d1<= MAXDIST 
        ...
        %           vn2 <-vid2 <- vp2                  vn2
        eid = eid1;
        eid_last = next_edge(eid2, opphvs);
        ...
        eid = eid2;
        eid_last = next_edge(eid1, opphvs);
    ...
    while eid ~= eid_last
        ...
        nv_old = nv;
        [nv, ne, edgs, opphvs, v2hv, vvs, evs] = contract_edge_curv(eid, nv, ne, ...
            edgs, opphvs, v2hv, vvs, evs);
...
if vid1~=nv || vid2~=nv-1
    [edgs, opphvs, v2hv] = swap_vertices_curv( vid1, nv-1, edgs, opphvs, v2hv);
    vvs(vid1,:) = vvs(nv-1,:);
    [edgs, opphvs, v2hv] = swap_vertices_curv( vid2, nv, edgs, opphvs, v2hv);
    vvs(vid2,:) = vvs(nv,:);
end

if edgs(eid1,1)~=nv-2 || edgs(eid2,1) ~= nv-3
    vp1 = edgs(eid1,1);
    [edgs, opphvs, v2hv] = swap_vertices_curv( vp1, nv-3, edgs, opphvs, v2hv);
    vvs(vp1,:) = vvs(nv-3,:);
    
    vp2 = edgs(eid1,1);
    [edgs, opphvs, v2hv] = swap_vertices_curv( vp2, nv-2, edgs, opphvs, v2hv);
    vvs(vp2,:) = vvs(nv-2,:);
end

%% Swap eid1 and eid2 to end of edge list.
if eid1~=ne || eid2~=ne-1
    [edgs, opphvs, v2hv] = swap_edges_curv( eid1, ne-1, edgs, opphvs, v2hv);
    evs(eid1,:) = evs(ne-1,:); eid1 = ne-1;

    [edgs, opphvs, v2hv] = swap_edges_curv( eid2, ne, edgs, opphvs, v2hv);
    evs(eid2,:) = evs(ne,:);   eid2 = ne;
end

% Swap adjacent vertices to the end
en1 = next_edge( eid1, opphvs); en2 = next_edge( eid2, opphvs);
...
if en1~=ne-2 || en2 ~= ne-3
    ...
    [edgs, opphvs, v2hv] = swap_edges_curv( en1, ne-3, edgs, opphvs, v2hv);
    evs(en1,:) = evs(ne-3,:); en1 = ne-3;

    % Swap en2 to end of edge list.
    [edgs, opphvs, v2hv] = swap_edges_curv( en2, ne-2, edgs, opphvs, v2hv);
    evs(en2,:) = evs(ne-2,:); en2 = ne-2;
end

%% Update edgs, opphvs, and v2hv
ep1 = prev_edge( eid1, opphvs); ep2 = prev_edge( eid2, opphvs);
enn1 = next_edge( en1, opphvs); enn2 = next_edge( en2, opphvs);

edgs(ep1, 2)=edgs(enn2, 1); opphvs(ep1, 2)=2*enn2; opphvs(enn2, 1)=2*ep1+1;
edgs(ep2, 2)=edgs(enn1, 1); opphvs(ep2, 2)=2*enn1; opphvs(enn1, 1)=2*ep2+1;
...
function d = distance( eid1, eid2, opphvs, maxd)
